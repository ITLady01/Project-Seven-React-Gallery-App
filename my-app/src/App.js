...
export default class App extends Component {
  ...
  render() {
      ...
      return (<div className = "container" > {
         ...
         otherwise I didn't need to use Route since the Header component will be always visible in the app */
        } <Route render = {({history}) => < Header handleFetching = {this.handleFetching}
          history = {history}/> }/> 
          ...
          <Switch> {
           ...
           <Route path = "/dogs" render = {() => < Photos data = {this.state.dogs}
            results = "Dogs" /> } />  <Route path = "/cats" render = {() => 
            < Photos data = {this.state.cats} results = "Cats"/>} /> 
            < Route path = "/birds" render = {() => < Photos data = {this.state.birds }
            results = "Birds"/>} /> <Route path = "/search /: query " render = {
            ({ match }) => (this.state.loading) ?
            <h2 > Loading... </h2> : < Photos data = {this.state.search}results = {match.params.query} match = {match } /> }/> 
